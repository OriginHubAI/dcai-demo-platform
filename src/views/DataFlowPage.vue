<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-900">DataFlow</h1>
      <span class="text-sm text-gray-500">{{ totalItems }} packages</span>
    </div>
    <!-- Category tabs -->
    <div class="flex items-center space-x-2 mb-6 overflow-x-auto pb-2">
      <button
        v-for="cat in dataflowCategories"
        :key="cat.value"
    ...
    <div class="flex items-center gap-3 mb-6">
      <div class="flex-1 max-w-md">
        <SearchBar v-model="searchQuery" placeholder="Search packages..." />
      </div>
      <SortDropdown v-model="sortBy" :options="packageSortOptions" />
    </div>
    <!-- Results -->
...
import { dataflowPackages, dataflowCategories } from '@/data/dataflow.js'
